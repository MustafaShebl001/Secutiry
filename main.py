...
class FileSecurityApp:
    def __init__(self, root):
        ...
        style = ttk.Style()
        style.configure("Rounded.TButton", borderwidth=5, relief="ridge", padding=(10, 5))
        ...
        # Encryption Section
        button_encrypt = tk.Button(frm, text="AES 256 Encrypt", command=self.on_aes_encrypt_button_click, bg="#4CAF50", fg="white",borderwidth=3)
        button_encrypt.grid(row=2, column=0, pady=20, padx=20)

        # Decryption Section
        button_decrypt = tk.Button(frm, text="AES 256 Decrypt", command=self.on_aes_decrypt_button_click, bg="#008CBA", fg="white",borderwidth=3)
        button_decrypt.grid(row=2, column=1, pady=20, padx=10)

         # Triple Des encryption
        button_des3_enc = tk.Button(frm, text="DES3 Enc",command=self.on_des_encrypt_button_click, bg="#4CAF50", fg="white",borderwidth=3)
        button_des3_enc.grid(row=3, column=0, pady=20)

        # Triple Des decryption
        button_des3_dec = tk.Button(frm, text="DES3 Dec",command=self.on_des_decrypt_button_click, bg="#008CBA", fg="white",borderwidth=3)
        button_des3_dec.grid(row=3, column=1, pady=20)


        # Key Pair Generation Section
        button_generate_keys = tk.Button(frm, text="Generate Keys",command=self.on__generate_keys_button_click, bg="#FFC107", fg="black",borderwidth=3)
        button_generate_keys.grid(row=4, column=0, columnspan=2, pady=20)

        # Digital Signature Section
        button_sign = tk.Button(frm, text="Sign A Document", command=self.on_sign_button_click, bg="#673AB7", fg="white",borderwidth=3)
        button_sign.grid(row=5, column=0, pady=20)

        button_verify = tk.Button(frm, text="Verify Signature",command=self.on_verify_button_click, bg="#FF5722", fg="white",borderwidth=3)
        button_verify.grid(row=5, column=1, pady=20)

        button_sign_and_encrypt = tk.Button(frm, text="Sign and Encrypt",command=self.on_sign_and_encrypt_button_click, bg="#E91E63", fg="white",borderwidth=3)
        button_sign_and_encrypt.grid(row=6, column=0, columnspan=2, pady=20)

        # Verify and Decrypt Section
        button_verify_and_decrypt = tk.Button(frm, text="Verify and Decrypt",  command=self.on_verify_and_decrypt_button_click, bg="#795548", fg="white",borderwidth=3)
        button_verify_and_decrypt.grid(row=7, column=0, columnspan=2, pady=20)


        button_cert = tk.Button(frm, text="Generate self-signed cert",  command=self.on_generate_certificate, bg="#FFEB3B", fg="black",borderwidth=3)
        button_cert.grid(row=8, column=0, pady=20)

        button_verify_cert = tk.Button(frm, text="Verify Cert.", command=self.on_verify_certificate, bg="#607D8B", fg="white")
    ...
    def on_verify_certificate():
        Cert_file_path = filedialog.askopenfilename(title="Select Certificate to verify")
        ...
        Authority_key_file = filedialog.askopenfilename(title="Select Authority public key")
        ...
        try:
            ...
            messagebox.showerror("Error Cert is unverified","This certificate is not verified")
            return
    ...
    def on_sign_and_encrypt_button_click():
        ...
        try:
            ...
            messagebox.showinfo("File is Encrypted","File is encrypted successfully")
        except Exception as e:
    ...
    def on_verify_button_click():
        ...
        file_path_to_verify = filedialog.askopenfilename(title="Select File to Verify")
